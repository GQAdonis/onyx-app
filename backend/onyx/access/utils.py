...
def prefix_user_file(user_file_id: int) -> str:
    """Prefixes a user file id to eliminate collision with group names."""
    return f"user_file:{user_file_id}"


def prefix_user_folder(user_folder_id: int) -> str:
    """Prefixes a user folder id to eliminate collision with group names."""
    return f"user_folder:{user_folder_id}"
